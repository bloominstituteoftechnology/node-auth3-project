...
server.post('/api/register', async (req, res, next) => {
    ...
    if (!username || !password || !deptcode) {
        return res.status(400).json(['Error-dataShape',
        {'username': 'REQUIRED / UNIQUE', 'password': 'REQUIRED', 'deptcode': 'REQUIRED',}]
        )
    }
    // const  checkIfDeptIsValid = () => {
    //     db('deptcodes').select('code').where('code', '=', deptcode).then(result => {
    //         if (!result[0]) {            
    ...
    const newUser = {username, password, deptcode};
