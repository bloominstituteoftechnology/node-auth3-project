...
const checkDepo = (department) => {
    return function(req, res, next) {
        console.log(req.decodedToken);
      if (req.decodedToken && req.decodedToken.department === department) {
        next();
      } else {
        res.status(403).json({ message: `You don't have access to this bro.` });
      }
    };
  }

/// ---- Non-middleware Functions ----
const jwtSecret = 'Catch me if you can...';
...
server.post('/login', (request, response)  => {
    ...
    .then( user => {
        ...
        if (user && bcrypt.compareSync(credentials.password, user.password)) {
            ...
            response.status(200).json({ authorized: `${username} has been logged in.`, token })
        } else {
...
server.get('/api/users', protected, checkDepo('admin'), (request, response) => {
    // Database Promise Methods
    db('user')
    .select('id', 'username')
    .then( users => {
